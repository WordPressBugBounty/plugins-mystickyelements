	...
	<div class="" >
		...
		<div class="myStickyelements-container myStickyelements-social-channels-tabs">
			...
			<div class="myStickyelements-header-title">
				...
				<label for="myStickyelements-social-channels-enabled" class="myStickyelements-switch large-switch">
				...
				<p class="social-disable-info" style="display: none;">
                    <i class="fas fa-info-circle"></i>
                    <span>
                        <?php esc_html_e('Social channels in sticky bar has been turned off.', 'mystickyelements');?>
                    </span>
                    <a href="#" class="mystickyelements-turnit-on" data-turnit="myStickyelements-social-channels-enabled">
			...
			<div class="mystickyelements-disable-wrap">
				...
				<div class="mystickyelements-action-popup-open mystickyelements-action-popup-status" id="socialform-status-popup" style="display:none;">
					<div class="popup-ui-widget-header">
						<span id="ui-id-1" class="ui-dialog-title"><?php esc_html_e('Disable Social Channels?', 'mystickyelements');?></span><span class="close-dialog" data-from ='social-form'> &#10006 </span>
					</div>	
					<div id="widget-delete-confirm" class="ui-widget-content">
						<p>
							<?php
                            _e("You are about to disable the <span>social channels and chat options</span>. Once disabled, they will no longer be visible. Only the contact form will appear if it is enabled.", 'mystickyelements');
                            ?>
						</p>
					</div>
					<div class="popup-ui-dialog-buttonset flex justify-end gap-4">
                        <button type="button" class="btn-disable-cancel button-social-popup-disable mse-secondary-button">
                            <?php esc_html_e('Disable anyway', 'mystickyelements');?>
                        </button>
                        <button type="button" class="mystickyelement-keep-widget-btn button-social-popup-keep mse-primary-button" data-from = "contact-form" >
        ...
        ?>
									<li data-search="<?php echo str_replace("_", " ", $search_channel); ?>" <?php if (isset($value['is_locked']) && $value['is_locked'] == 1): ?> class="upgrade-myStickyelements" <?php endif; ?>>
										<label>
											...
											<input type="checkbox" data-social-channel-text="<?php echo esc_attr($value['hover_text']);?>" data-social-channel="<?php echo esc_attr($key); ?>" class="social-channel" name="social-channels[<?php echo esc_attr($key); ?>]" value="1" <?php checked(@$social_channels[$key], '1'); ?> <?php if (isset($value['is_locked']) && $value['is_locked'] == 1) {
											    echo "disabled";
											} ?>/>
										</label>
    ...
    endforeach; ?>						
				...
				<div class="mystickyelement-social-buttons">
					<div class="mystickyelement-more-less-channel">
						<a id="myStickyelements-more-social" class="mse-primary-button" href="#">
					...
					<div class="mystickyelement-custom-channel">
						<button id="myStickyelements-add-custom-social" name="button_add_custom_social" value = "<?php _e('Custom Channel', 'mystickyelements'); ?>"><?php _e('Custom Channel', 'mystickyelements'); ?>
                            <svg id="plus-circle" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 36 36">
                                <path id="Path_1928" data-name="Path 1928" d="M18,7.875A1.125,1.125,0,0,1,19.125,9v9A1.125,1.125,0,0,1,18,19.125H9a1.125,1.125,0,0,1,0-2.25h7.875V9A1.125,1.125,0,0,1,18,7.875Z" fill-rule="evenodd" fill="currentColor"/>
                                <path id="Path_1929" data-name="Path 1929" d="M16.875,18A1.125,1.125,0,0,1,18,16.875h9a1.125,1.125,0,0,1,0,2.25H19.125V27a1.125,1.125,0,0,1-2.25,0Z" fill-rule="evenodd" fill="currentColor"/>
                                <path id="Path_1930" data-name="Path 1930" d="M18,33.75A15.75,15.75,0,1,0,2.25,18,15.75,15.75,0,0,0,18,33.75ZM18,36A18,18,0,1,0,0,18,18,18,0,0,0,18,36Z" fill-rule="evenodd" fill="currentColor"/>
                            </svg>
                        </button>
						<label>
                            ...
                            <a class = "custom-social-channel-info-link" href="https://premio.io/help/mystickyelements/how-do-i-create-a-custom-link-or-javascript-channel/" target="_blank"><?php _e('How custom channels work?', 'mystickyelements'); ?></a>
						</label>
					</div>
					<div class="mystickyelement-custom-shortcode">
						<button id="myStickyelements-add-custom-shortcode" name="button_add_custom_shortcode" value= "<?php _e('Custom Shortcode', 'mystickyelements'); ?>"  >
                            <?php _e('Custom Shortcode', 'mystickyelements'); ?>
                            <svg id="plus-circle" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 36 36">
                                <path id="Path_1928" data-name="Path 1928" d="M18,7.875A1.125,1.125,0,0,1,19.125,9v9A1.125,1.125,0,0,1,18,19.125H9a1.125,1.125,0,0,1,0-2.25h7.875V9A1.125,1.125,0,0,1,18,7.875Z" fill-rule="evenodd" fill="currentColor"/>
                                <path id="Path_1929" data-name="Path 1929" d="M16.875,18A1.125,1.125,0,0,1,18,16.875h9a1.125,1.125,0,0,1,0,2.25H19.125V27a1.125,1.125,0,0,1-2.25,0Z" fill-rule="evenodd" fill="currentColor"/>
                                <path id="Path_1930" data-name="Path 1930" d="M18,33.75A15.75,15.75,0,1,0,2.25,18,15.75,15.75,0,0,0,18,33.75ZM18,36A18,18,0,1,0,0,18,18,18,0,0,0,18,36Z" fill-rule="evenodd" fill="currentColor"/>
				...
				<div class="social-channel-popover mse-shadow" style="display:none;">
					<div>
						...
						<div class="pt-5">
							<a class="mse-upgrade-now-button large-button" href="<?php echo esc_url($upgrade_url); ?>" target="_blank" >
                                Upgrade Now
...
<?php
    ...
    function social_channel_icon_class($key, $value)
    {
        ...
        } elseif (strpos($key, 'custom_channel') !== false && $value['custom_icon'] == '' && $value['fontawesome_icon'] != '') {
            return $value['fontawesome_icon'];
        } elseif (strpos($key, 'custom_shortcode') !== false && $value['custom_icon'] == '' && $value['fontawesome_icon'] == '') {
            return 'fas fa-code';
        } elseif (strpos($key, 'custom_shortcode') !== false && $value['custom_icon'] == '' && $value['fontawesome_icon'] != '') {
            return $value['fontawesome_icon'];
...
function set_span_bg_color($key, $value)
{
    ...
    if (strpos($key, 'custom_channel') !== false || in_array($key, $custom_channel)) {
        return '#7761DF';
    } elseif (strpos($key, 'custom_shortcode') !== false || in_array($key, $custom_shortcode)) {
        return '#7761DF';
    } else {
