		...
		<div class="myStickyelements-preview-screen">
			...
			<div class="preview-element-contact-form <?php echo esc_attr($general_settings['position'])?>_pos  <?php echo esc_attr($general_settings['position_mobile'])?>_pos_mobile  ">
                <div class="preview-element-contact-form-wrap">
                    ...
                    <h2 style="color:<?php if ((isset($contact_form['headine_text_color']))) {
                        echo esc_attr($contact_form['headine_text_color']);
                    } ?>"><?php echo esc_html($contact_form['contact_title_text']); ?></h2>
					<div class="preview-element-contact-form-wrap-div">
                    <input type="text" id="contact-form-name" class = "preview-contactform-field" placeholder="<?php echo (isset($contactform['name_value']) && $contactform['name_value'] != '') ? $contactform['name_value'] : 'Name';?>" data-slug = "name" style ="<?php if (!isset($contact_form['name']) || (isset($contact_form['name']) && $contact_form['name'] != '1')) {
                        echo "display:none";
                    } else {
                        echo "display:block";
                    } ?>" >
        
                    <input type="text" id="contact-form-phone" class = "preview-contactform-field" placeholder="<?php echo (isset($contactform['phone_value']) && trim($contactform['phone_value']) != '') ? $contactform['phone_value'] : 'Phone';?>" data-slug="phone" style ="<?php if (!isset($contact_form['phone']) || (isset($contact_form['phone']) && $contact_form['phone'] != '1')) {
                        echo "display:none";
                    } else {
                        echo "display:block";
                    } ?>">

                    <input type="text" id="contact-form-email" class = "preview-contactform-field" placeholder="<?php echo (isset($contactform['email_value']) && $contactform['email_value'] != '') ? $contactform['email_value'] : 'Email';?>" data-slug="email" style ="<?php if (!isset($contact_form['email']) || (isset($contact_form['email']) && $contact_form['email'] != '1')) {
                        echo "display:none";
                    } else {
                        echo "display:block";
                    } ?>">

                    <textarea  class = "preview-contactform-field" id="contact-form-message" placeholder="Message"  data-slug="message" style ="<?php if (!isset($contact_form['message']) || (isset($contact_form['message']) && $contact_form['message'] != '1')) {
                        echo "display:none";
                    } else {
                        echo "display:block";
                    } ?>"></textarea>
                    
                    <select name="contact-form[dropdown_value]" id="contact-form-dropdown-value" style ="<?php if (!isset($contact_form['dropdown']) || (isset($contact_form['dropdown']) && $contact_form['dropdown'] != '1')) {
                        echo "display:none";
                    } else {
                        echo "display:block";
                    } ?>">
                        <option value=""><?php if ((isset($contact_form['dropdown-placeholder']))) {
                    ...
                    <input type="submit" id="preview-stickyelements-submit-form" style="background:<?php if ((isset($contact_form['submit_button_background_color']))) {
                        echo esc_attr($contact_form['submit_button_background_color']);
                    } else {
                        echo "#7761DF";
                    }?>; color:<?php if ((isset($contact_form['submit_button_text_color']))) {
                        echo esc_attr($contact_form['submit_button_text_color']);
                    } else {
                        echo "#FFFFFF";
                    }?>;" value="<?php if ((isset($contact_form['submit_button_text']))) {
                        echo esc_attr($contact_form['submit_button_text']);
                    } else {
                        echo "Submit";
                    }?>" disabled>
					</div>	
            ...
            <div id="mystickyelements-preview" class="mystickyelements-preview-tab mystickyelements-fixed <?php echo esc_attr((isset($contact_form['direction']) && $contact_form['direction'] == "RTL") ? "is-rtl" : "") ?> mystickyelements-position-<?php echo esc_attr($general_settings['position'])?> <?php echo esc_attr((isset($general_settings['position_on_screen']) && $general_settings['position_on_screen'] != '') ? 'mystickyelements-position-screen-' .$general_settings['position_on_screen'] : 'mystickyelements-position-screen-center');?> mystickyelements-position-mobile-<?php echo esc_attr($general_settings['position_mobile'])?> <?php echo esc_attr((isset($general_settings['widget-size']) && $general_settings['widget-size'] != '') ? 'mystickyelements-size-' .$general_settings['widget-size'] : 'mystickyelements-size-medium');?> <?php echo esc_attr((isset($general_settings['mobile-widget-size']) && $general_settings['mobile-widget-size'] != '') ? 'mystickyelements-mobile-size-' .$general_settings['mobile-widget-size'] : 'mystickyelements-mobile-size-medium');?> <?php echo esc_attr((isset($general_settings['entry-effect']) && $general_settings['entry-effect'] != '') ? 'mystickyelements-entry-effect-' .$general_settings['entry-effect'] : 'mystickyelements-entry-effect-slide-in');?> <?php echo esc_attr((isset($general_settings['templates']) && $general_settings['templates'] != '') ? 'mystickyelements-templates-' .$general_settings['templates'] : 'mystickyelements-templates-default');?> ">
...
?>
		...
		<p class="description text-center text-[#364153] rounded-sm" id="myStickyelements_mobile_templete_desc" style="display: none;">
			<strong><?php esc_html_e('The default template is the only template that is currently available for the mobile top/bottom position', 'mystickyelements');?></strong>
		</p>
