...
if (!class_exists('MyStickyElementsFrontPage_pro')) {
    ...
    class MyStickyElementsFrontPage_pro
    {
        ...
        public function mystickyelements_enqueue_script()
        {
			...
			<?php
            wp_enqueue_script('mailcheck-js', plugins_url('dist/js/mailcheck.js', __FILE__), ['jquery'], MY_STICKY_ELEMENT_VERSION, ['strategy'  => 'defer', 'in_footer' => true ]);
        ...
        public function mystickyelement_element_footer()
        {
            ...
            $mystickyelements_class[] = 'mse-wrap';
            ...
            ?>
						...
						<?php if (isset($contact_form['enable']) && $contact_form['enable'] == 1):
							...
							<li id="mystickyelements-contact-form"
                                style="border-color: <?php echo esc_attr($border_color) ?>; border-width: <?php echo esc_attr($border_width) ?>px"
                                class="mystickyelements-contact-form <?php echo esc_attr($contact_form_class); ?>"  <?php if (isset($contact_form['direction']) && $contact_form['direction'] == 'RTL') : ?> dir="rtl" <?php endif; ?> data-tab-opt="<?php echo esc_attr($general_settings["open_tabs_when"]); ?>" data-time-delay="<?php echo esc_attr($form_open_delay_sec);?>" >
								<?php
								...
								<div class="element-contact-form" style="background-color: <?php echo esc_attr((isset($contact_form['form_bg_color'])) ? $contact_form['form_bg_color'] : '#ffffff'); ?>">
									...
									<form id="stickyelements-form" class="stickyelements-form" action="" method="post" autocomplete="off" data-close-after="<?php echo esc_attr($close_after); ?>">
										<?php foreach ($contact_field as $value) :
											...
											<label>
												<span class="sr-only"><?php echo esc_html($contact_form['name_value']);?></span>
												<input
													class="<?php if (isset($contact_form['name_require']) && $contact_form['name_require'] == 1): ?> required<?php endif; ?>"
													type="text" id="contact-form-name" name="contact-form-name" value=""
													placeholder="<?php echo esc_attr($contact_form['name_value']);
										                if (isset($contact_form['name_require']) && $contact_form['name_require'] == 1): esc_html_e("*", 'mystickyelements'); endif; ?>"  <?php if (isset($contact_form['name_require']) && $contact_form['name_require'] == 1): ?> required<?php endif; ?> autocomplete="off"/>
											</label>
										<?php endif;
										            break;
										        case 'phone':
											...
											<label>
												<span class="sr-only"><?php echo esc_html($contact_form['phone_value']);?></span>
												<input
													class="<?php if (isset($contact_form['phone_require']) && $contact_form['phone_require'] == 1): ?> required<?php endif; ?>"
													type="tel" id="contact-form-phone" name="contact-form-phone" value="" oninput="this.value = this.value.replace(/[^0-9+]/g, '');"
													placeholder="<?php echo esc_attr($contact_form['phone_value']);
										                if (isset($contact_form['phone_require']) && $contact_form['phone_require'] == 1): esc_html_e("*", 'mystickyelements'); endif; ?>" <?php if (isset($contact_form['phone_require']) && $contact_form['phone_require'] == 1): ?> required <?php endif; ?> autocomplete="off" />
											</label>
										...
										<?php endif;
											...
											<label>
												<span class="sr-only"><?php echo esc_html($contact_form['email_value']);?></span>
												<input
													class="email <?php if (isset($contact_form['email_require']) && $contact_form['email_require'] == 1): ?> required<?php endif; ?>"
													type="email" id="contact-form-email" name="contact-form-email" value=""
													placeholder="<?php echo esc_attr($contact_form['email_value']);
										                if (isset($contact_form['email_require']) && $contact_form['email_require'] == 1): esc_html_e("*", 'mystickyelements'); endif; ?>" <?php if (isset($contact_form['email_require']) && $contact_form['email_require'] == 1): ?> required <?php endif; ?> autocomplete="off"/>
											</label>
										<?php endif;
										            break;
										        case 'message':
											...
											<label>
												<span class="sr-only"><?php echo esc_html($contact_form['message_value']);?></span>
												<textarea
													class="<?php if (isset($contact_form['message_require']) && $contact_form['message_require'] == 1): ?> required<?php endif; ?>"
													id="contact-form-message" name="contact-form-message"
													placeholder="<?php echo esc_attr($contact_form['message_value']);
										                if (isset($contact_form['message_require']) && $contact_form['message_require'] == 1): esc_html_e("*", 'mystickyelements'); endif; ?>" <?php if (isset($contact_form['message_require']) && $contact_form['message_require'] == 1): ?> required <?php endif; ?>></textarea>
											</label>
										<?php endif;
										            break;
										        case 'dropdown':
												...
												<label>
													<span class="sr-only"><?php echo esc_html($contact_form['dropdown-placeholder']);?></span>
            ...
            if (!empty($social_channels_tabs) && isset($social_channels['enable']) && $social_channels['enable'] == 1) :
                $protocols = apply_filters('mystickyelements_esc_js_protocols', array('http', 'https', 'mailto', 'tel', 'sms','viber','skype', 'javascript'));
                    ...
                    ?>
								...
								<?php else :
									...
									<span class="mystickyelements-social-text <?php echo esc_attr(($social_link == '') ? 'mystickyelements-social-no-link' : '');?>" style= "<?php echo esc_attr($icon_bg_color.$icon_text_color) ?>" >
										<?php if ($social_link != ''): ?>
										<a class="social-link-<?php echo esc_attr($key);?>" href="<?php echo esc_url($social_link, $protocols); ?>"  <?php if ($link_target == 1):?> target="_blank" rel="noopener nofollow" <?php endif;?> <?php if (isset($value['icon_color']) && $value['icon_color'] != '') : echo "style='color:" . esc_attr($value['icon_color']) . "'"; endif; ?> data-tab-setting = '<?php echo esc_attr((isset($general_settings["open_tabs_when"]) && $general_settings["open_tabs_when"] != "") ? $general_settings["open_tabs_when"] : "");?>' data-flyout="<?php echo esc_attr((isset($general_settings['flyout']) && $general_settings['flyout'] != '') ? $general_settings['flyout'] : '') ?>" title="<?php echo (isset($value['icon_text']) && $value['icon_text'] != '') ? esc_attr($value['icon_text']) : esc_attr(stripslashes($hover_text));?>"
        ...
        public function mystickyelements_contact_form()
        {
            ...
            if (isset($stickyelements_widgets[0]['status'])  && $stickyelements_widgets[0]['status'] == 0) {
            ...
            if (isset($contact_form['email']) && $contact_form['email'] == 1) {
                if (isset($contact_form['email_require']) && $contact_form['email_require'] == 1 && (!isset($_POST['contact-form-email']) || empty($_POST['contact-form-email']))) {
                    ...
                    $errors[] = $error;
                } elseif (isset($contact_form['email_require']) && $contact_form['email_require'] == 1 && isset($_POST['contact-form-email']) && !filter_var($_POST['contact-form-email'], FILTER_VALIDATE_EMAIL)) {
                    $error = array(
                        'key' => "contact-form-email",
            ...
            if (empty($errors)) {
                if (!isset($_POST['nonce']) || empty($_POST['nonce'])) {
                    ...
                    $errors[] = $error;
                } elseif (!isset($_POST['form_id']) || empty($_POST['form_id'])) {
                    $error = array(
                        'key' => "mse-form-error",
                        'message' => "There is error. We are not able to complete your request"
                    );
                    $errors[] = $error;
                } elseif (!wp_verify_nonce($_POST['form_id'], $_POST['nonce'])) {
                    $error = array(
                        'key' => "mse-form-error",
                        'message' => "There is error. We are not able to complete your request"
                    );
        ...
        public function get_user_ipaddress()
        {
