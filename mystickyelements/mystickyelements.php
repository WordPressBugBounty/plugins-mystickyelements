<?php
/*
Plugin Name: My Sticky Elements
Plugin URI: https://premio.io/
Description: My Sticky Elements is simple yet very effective plugin. It is perfect to fill out usually unused side space on webpages with some additional messages, videos, social widgets ...
Version: 2.3.9
Author: Premio
...
define("MY_STICKY_ELEMENT_VERSION", "2.3.9");
...
if (!function_exists("check_for_mse_redirect")) {
    function check_for_mse_redirect()
    {
        if (!defined("DOING_AJAX")) {
            if (get_option("mse_redirect")) {
    ...
    add_action("admin_init", "check_for_mse_redirect");
}
...
class MyStickyElementsPage
{
    ...
    public function sanitize($input)
    {
        ...
        if (isset($input['myfixed_element_content'])) {
            $new_input['myfixed_element_content'] =  wp_kses(
                $input['myfixed_element_content'],
                array(
                'a' => array(
                        'href' => array(),
                        'title' => array(),
    ...
    function mystickyelements_social_channels()
    {
        $social_channels = array(
                            ...
                            'insagram_dm'           => [
                                            ...
                                            'class'            => "mystickyelement_insagramdm_icon",
                            ...
                            'snapchat'	=> array(
                                            ...
                                            'tooltip'	=> 'Enter your Snapchat username. E.g., snapchat_username',
                                            'icon_color' => 1
                                        ),
                            ...
                            'twitter'	=> array(
                                            ...
                                            'search_class' => "X",
                                            'background_color' => "#000000",
                                            'placeholder'	=> 'Example: https://x.com/cocacola',
                                            'class' => "fa-brands fa-x-twitter",
                                            'tooltip'	=> 'Add the link of your X profile E.g., <a href="https://x.com/cocacola" target="_blank">https://x.com/cocacola</a>',
                                            'icon_color' => 1
                                        ),
                            'linkedin'	=> array(
                                            'text' => "LinkedIn",
                            ...
                            'pinterest'	=> array(
                                            ...
                                            'background_color' => "#E85F65",
                                            'placeholder'	=> 'Example: https://pinterest.com/username',
                            ...
                            'trip_advisor'	=> array(
                                            ...
                                            'icon_color' => 1,
                                            'custom_svg_icon'  => file_get_contents(MYSTICKYELEMENTS_PATH.'/images/tripadvisor-icon.svg'),
                                        ),
                            'medium'	=> array(
                                            'text' => "Medium",
                                            'icon_text' => "",
                            ...
                            'printful'	=> array(
                                            ...
                                            'hover_text' => "Printful",
                                            'background_color' => "#000",
                                            'placeholder'	=> 'Example: https://www.printful.com/your_product',
                                            'class' => "mystickyelement_printful_icon",
                                            'tooltip'	=> 'Add your Printful product link. E.g, <a href="https://www.printful.com/your_product" target="_blank">https://www.printful.com/your_product</a>',
                                            'icon_color' => 1
                                        ),
                            'gumroad'	=> array(
                                            'text' => "Gumroad",
                            ...
                            'ok'		=> array(
                                            ...
                                            'placeholder'	=> 'Example: https://ok.ru/your_profile',
                                            'class' => "fab fa-odnoklassniki",
                                            'tooltip'	=> 'Add your full profile link of Ok.ru. E.g, <a href="https://ok.ru/your_profile" target="_blank">https://ok.ru/your_profile</a>',
                                            'icon_color' => 1
                                        ),
...
function mystickyelements_change_menu_text()
{
    global $submenu;
    if (isset($submenu['my-sticky-elements'])) {
        $totalItems = count($submenu['my-sticky-elements']) - 1;
        if (isset($submenu['my-sticky-elements'][$totalItems][0])) {
            $submenu['my-sticky-elements'][$totalItems][0] = '<span><svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
</svg></span> '.esc_html__('Upgrade to Pro', 'mystickyelements');
        }
    }
}
add_action('admin_init', 'mystickyelements_change_menu_text');
...
function mystickyelements_admin_init()
{
    ...
    if ($chatway_action == 'download_csv_file' && !empty($nonce) && wp_verify_nonce($nonce, 'stickyelement-download-csv-file') && current_user_can('activate_plugins')) {
        $table_name = $wpdb->prefix . "mystickyelement_contact_lists";
        $query = "SELECT * FROM {$table_name} ORDER BY ID DESC";
        $result     = $wpdb->get_results($query);
        $file = "mystickyelements-contact-leads.csv";
        $fp = fopen($file, "a") or die("Error Couldn't open $file for writing!");
        $all_data = '';
        $fields = array("ID", "Widget Name", "Name", "Phone", "Email", "Message", "Page Link", "Message Date");
        fputcsv($fp, $fields, ',', '"', '\\');
        foreach ($result as $res) {
            ...
            $fields = array($res_ID, $widget_element_name, $res_name, $res_phone, $res_email, $res_message, $page_link ,$res_message_date);
    ...
    if ($pagenow == 'plugins.php'  || (isset($_GET['page']) && $_GET['page'] == 'my-sticky-elements-analytics')) {

        include_once ABSPATH.'wp-admin/includes/upgrade.php';
        $charset_collate = $wpdb->get_charset_collate();

        $mystickyelement_widget_analytics_table     = $wpdb->prefix.'mystickyelement_widget_analysis';

        if ($wpdb->get_var("show tables like '{$mystickyelement_widget_analytics_table}'") != $mystickyelement_widget_analytics_table) {

            $mystickyelement_widget_analytics_sql = "CREATE TABLE {$mystickyelement_widget_analytics_table} (
            ...
            dbDelta($mystickyelement_widget_analytics_sql);
        }
    }
...
function mystickyelement_default_settings($section)
{
...
add_action('admin_footer', 'mystickyelements_admin_footer_style');
function mystickyelements_admin_footer_style()
{
    ?>
	<style>
    #adminmenu .toplevel_page_my-sticky-elements > ul > li:last-child {
        padding: 5px 10px;
    }
    #adminmenu .toplevel_page_my-sticky-elements > ul > li:last-child a {
        display: flex;
    ...
    #adminmenu .toplevel_page_my-sticky-elements > ul > li:last-child a:hover, #adminmenu .toplevel_page_my-sticky-elements > ul > li:last-child a.current {
        box-shadow: 0px 6px 8px 0px #B78DEB3D;
        color: #ffffff;
        background-color: #9565d0;
        font-weight: normal;
    }
    #adminmenu .toplevel_page_my-sticky-elements > ul > li:last-child a span {
        flex: 0 0 16px;
        height: 16px;
    ...
    #adminmenu .toplevel_page_my-sticky-elements > ul > li:last-child a:hover span {
        background-color: #B78DEB;
    }
    #adminmenu .toplevel_page_my-sticky-elements > ul > li:last-child a span svg {
        width: 100%;
        height: 100%;
    }
	</style>
